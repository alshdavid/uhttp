...
use super::response::Http1ResponseTx;
use crate::HandlerFunc;

/// Simple wrapper around hyper to make it a little nicer to use
pub fn create_server<Fn: HandlerFunc>(handle_func: Fn) -> Server<Fn> {
  Server(handle_func)
}

pub struct Server<Fn: HandlerFunc>(Fn);

impl<Fn: HandlerFunc> Server<Fn> {
  pub fn new(handle_func: Fn) -> Self {
    Self(handle_func)
  }
              ...
              service_fn({
                move |req| {
                  async move {
                    ...
                    // Drain the channel
                    let mut buf = vec![];
                    while let Ok(Http1ResponseTx::Write(bytes)) = rx_writer.try_recv() {
                      buf.extend(bytes);
                    }
                    ...
                    if content_type_event_stream || content_transfer_encoding_chunked {
                      ...
                      tokio::task::spawn(async move {
                        if let Some(Http1ResponseTx::Write(bytes)) = rx_writer.recv().await {
                    ...
                    if let Some(Http1ResponseTx::Write(bytes)) = rx_writer.recv().await {
                      ...
                      tokio::task::spawn(async move {
                        ...
                        while let Some(Http1ResponseTx::Write(bytes)) = rx_writer.recv().await {
                          writer.write(&bytes).await.unwrap();
                          writer.flush().await.unwrap();
                        }
